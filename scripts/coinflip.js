...
async function playCoinflip() {
  ...
  try {
    // Approve the Coinflip contract to spend the bet amount
    await polyPenToken.methods.approve(contractAddress, betAmountWei).send({
      from: accounts[0]
    });

    // Call the coinflip function from the Coinflip contract
    const tx = await coinflipContract.methods.coinflip(betAmountWei).send({
      from: accounts[0],
      value: web3.utils.toWei("0.1", "ether")
    });

    // Process the GameResult event
    const gameResultEvent = tx.events.GameResult.returnValues;
    const win = gameResultEvent.win;
    const wager = gameResultEvent.amount;

    // Display the game result
    const gameResultDiv = document.getElementById("gameResult");
    if (win) {
      gameResultDiv.innerHTML = `You won! You received ${web3.utils.fromWei(wager, 'ether')} PolyPen Tokens.`;
    } else {
      gameResultDiv.innerHTML = `You lost. Better luck next time!`;
    }
  } catch (err) {
